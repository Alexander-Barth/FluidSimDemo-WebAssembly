...
@inline @inbounds function diffusion(mask,(invΔx,invΔy),Du_over_Δxy,u,(i,j))
    ...
    return ((FXu2 - FXu1) * Du_over_Δxy[1] + (FYu2 - FYu1) * Du_over_Δxy[1])
end


@inline @inbounds function diffusion(mask::Nothing,(invΔx,invΔy),Du,u,(i,j))
end
...
function step!(config,mask,(u,v),(un,vn))
    ...
    invΔxy = (invΔx,invΔy)
    Du_over_Δxy = Du .* (invΔx,invΔy)
    Dv_over_Δxy = Dv .* (invΔx,invΔy)
